...
class KMeans:
    ...
    def fit(self, X):
        ...
        for _ in range(self.n_init):
            ...
            random_indices = np.random.choice(
                self.n_samples, size=self.K, replace=False
            )
    ...
    def _is_converged(self, prev_centroids, centroids, tol=1e-6):
        ...
        """
        return all(
            np.linalg.norm(prev_centroid - centroid) < tol
            for prev_centroid, centroid in zip(prev_centroids, centroids)
        )
    ...
    def _calculate_inertia(self, clusters, centroids):
        ...
        for cluster_idx, cluster in enumerate(clusters):
            cluster_inertia = np.sum(
                np.linalg.norm(self.X[cluster] - centroids[cluster_idx], axis=1) ** 2
            )
